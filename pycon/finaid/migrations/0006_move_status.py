...
class Migration(DataMigration):
    """Migration to move status field from the application to the review data, if needed"""

    def forwards(self, orm):
        """Copy non-default statuses from applications to review records"""
        FinancialAidApplication = orm['finaid.FinancialAidApplication']
        FinancialAidReviewData = orm['finaid.FinancialAidReviewData']
        STATUS_SUBMITTED = 1

        # We only need to do the migration if the status is not the default, 'submitted'
        for application in FinancialAidApplication.objects.exclude(status=STATUS_SUBMITTED):
            try:
    ...
    def backwards(self, orm):
        """Copy statuses from reviews back to applications"""
        FinancialAidApplication = orm['finaid.FinancialAidApplication']
        FinancialAidReviewData = orm['finaid.FinancialAidReviewData']

        for review in FinancialAidReviewData.objects.all():
