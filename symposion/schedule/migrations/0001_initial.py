...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        m2m_table_name = db.shorten_name('schedule_session_slots')
        db.create_table(m2m_table_name, (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('session', models.ForeignKey(orm['schedule.session'], null=False)),
            ('slot', models.ForeignKey(orm['schedule.slot'], null=False))
        ))
        db.create_unique(m2m_table_name, ['session_id', 'slot_id'])

        # Adding model 'SessionRole'
        db.create_table('schedule_sessionrole', (
            ('id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
        ...
        db.send_create_signal('schedule', ['SessionRole'])

        # Adding unique constraint on 'SessionRole', fields ['session', 'user', 'role']
        db.create_unique('schedule_sessionrole', ['session_id', 'user_id', 'role'])


    def backwards(self, orm):
        # Removing unique constraint on 'SessionRole', fields ['session', 'user', 'role']
        db.delete_unique('schedule_sessionrole', ['session_id', 'user_id', 'role'])
        ...
        db.delete_table(db.shorten_name('schedule_session_slots'))

        # Deleting model 'SessionRole'
        db.delete_table('schedule_sessionrole')
    ...
    models = {
        ...
        'schedule.session': {
            ...
            'day': ('django.db.models.fields.related.ForeignKey', [], {'related_name': "'sessions'", 'to': "orm['schedule.Day']"}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'slots': ('django.db.models.fields.related.ManyToManyField', [], {'related_name': "'sessions'", 'symmetrical': 'False', 'to': "orm['schedule.Slot']"})
        },
